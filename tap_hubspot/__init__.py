...
def parse_custom_schema(entity_name, data):
    return {
        ...
        for field in data
        if ("__c" != field["name"][(len(field["name"]) - 3) :])
    }
...
def sync_companies(STATE, ctx):
    ...
    STATE = singer.write_bookmark(
        STATE, "companies", bookmark_key, utils.strftime(max_bk_value)
    )
    singer.write_state(STATE)
    return STATE
...
def sync_engagements(STATE, ctx):
    ...
    if (
        has_bookmark(STATE, "engagements", bookmark_key)
        and last_sync_duration is not None
    ):
        LOGGER.info(
            (
...
def get_selected_streams(remaining_streams, annotated_schema):
    ...
    for stream in remaining_streams:
        ...
        selected_stream = next(
            (
                s
                for s in annotated_schema["streams"]
