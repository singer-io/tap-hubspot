...
def verify_environment_vars():
    missing_envs = [
        x
        for x in [
            os.getenv("TAP_HUBSPOT_REDIRECT_URI"),
            os.getenv("TAP_HUBSPOT_CLIENT_ID"),
            os.getenv("TAP_HUBSPOT_CLIENT_SECRET"),
            os.getenv("TAP_HUBSPOT_REFRESH_TOKEN"),
        ]
        if x == None
    ]
    if len(missing_envs) != 0:
        ...
        raise Exception(
            "set TAP_HUBSPOT_REDIRECT_URI, TAP_HUBSPOT_CLIENT_ID, TAP_HUBSPOT_CLIENT_SECRET, TAP_HUBSPOT_REFRESH_TOKEN"
        )
...
def get_clear_state():
    return {
        "bookmarks": {
            "contacts": {"offset": {}, "lastmodifieddate": None},
            "companies": {"offset": {}, "hs_lastmodifieddate": None},
        },
...
def our_write_bookmark(state, table_name, bookmark_key, bookmark_value):
    ...
    state = singer.bookmarks.write_bookmark(
        state, table_name, bookmark_key, bookmark_value
    )
    return state
